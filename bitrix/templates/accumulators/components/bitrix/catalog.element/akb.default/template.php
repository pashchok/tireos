...
if (!empty($_REQUEST['action']) && $_REQUEST['action'] === 'sendorderrequest') {
    global $USER;
    $product = CIBlockElement::GetByID($_REQUEST['product']);
    ...
    $text = '
        ...
        <body>
        <div id="picture">
            <h1><a href="http://' . SITE_SERVER_NAME . $product['DETAIL_PAGE_URL'] . '">' . $product['NAME'] . '</a></h1>
            <img src="http://' . SITE_SERVER_NAME . $product['PREVIEW_PICTURE']['SRC'] . '" />
    ...
    CEvent::SendImmediate(
        ...
        array(
            ...
            // 'SITE_NAME' => 'test',
            // 'SERVER_NAME' => 'test',
            'EMAIL_TO' => 'test2@no-spam.ws',
            // 'EMAIL_TO' => 'spirit@taganlife.ru',
        ),
        "N",
        ""
    );
    ...
    <div class="bx_item_container">
        <div class="bx_lt">
		  ...
		  <div class="<? echo $strClass; ?>" id="<? echo $arItemIDs['SLIDER_CONT_ID']; ?>">
			  ...
			  <div class="bx_slide<?if(count($arResult['MORE_PHOTO'])>3):?> carousel<?endif;?>">
			      <ul style="width: <? echo $strWidth; ?>;" id="<? echo $arItemIDs['SLIDER_LIST']; ?>">
	      <?
		      foreach ($arResult['MORE_PHOTO'] as &$arOnePhoto)
		      {
	      ?>
				  <li data-value="<? echo $arOnePhoto['ID']; ?>" ><span class="cnt"><span class="cnt_item" style="background-image:url('<? echo $arOnePhoto['SRC']; ?>');"></span></span></li>
	      <?
		      }
		      unset($arOnePhoto);
	      ?>
			  ...
			  <div class="prev jcarousel-control-prev"></div>
			  <div class="next jcarousel-control-next"></div>
              <?endif;?>
		      </div>
		  </div>
	      <?
	      ...
	      ?>
				      ...
				      <div class="fine-three">
					      <ul>
						      <li class="s1">Бесплатная доставка</li>
						      <li class="s2">Гарантия производителя</li>
						      <li class="s3">Установка и настройка</li>
					      </ul>
				      </div>
				      <? /*<div class="inst-three">
					      <ul>
						      <li><a href="#">Инструкция</a> <sup>1.5Мб</sup></li>
						      <li><a href="#">Сертификат</a></li>
						      <li><a href="#" class="show-review">Отзывы (<?php $count = $arResult['PROPERTIES']['BLOG_COMMENTS_CNT']['VALUE']; if ($count < 1) $count="0";  echo $count; ?>)</a></li>
					      </ul>
				      </div>*/?>
	    <div class="revuews-section-container">
	    <?
	    if ('Y' == $arParams['USE_COMMENTS'] || 1==1)
	    {
	    ?>
...
?>
    ...
    <div class="item_current_price" id="<? echo $arItemIDs['PRICE']; ?>"><span data-baseprice="<?=$arResult['MIN_PRICE']['DISCOUNT_VALUE']?>"><? echo $arResult['MIN_PRICE']['DISCOUNT_VALUE']; ?></span> руб.</div>
...
?>
    ...
    <div class="item_buttons vam">
       <!-- <span class="item_buttons_counter_block">
            ...
            --><input id="<? echo $arItemIDs['QUANTITY']; ?>" type="hidden" class="tac transparent_input" value="<? echo (isset($arResult['OFFERS']) && !empty($arResult['OFFERS'])
                    ? 1
        ...
        <span class="item_buttons_counter_block">
            <a href="javascript:void(0);" class="add_to_btn_detail <? echo $buyBtnClass; ?>" id="<? echo $arItemIDs['BUY_LINK']; ?>" data-id="<?=$arResult['ID']?>"><span></span><?=($arResult['CAN_BUY'] ? 'В корзину' : 'Заказать')?></a>
		
<?
if ($arResult['CATALOG_QUANTITY'] > 0):
?>
<a href="#" onClick="BocGetItem(<?=$arResult['ID']?>); return false;" class="oneclick_detail" title="купить в один клик"></a>
<? endif; ?>
...
<div style="clear:both"></div>
			...
			<!--div class="credit">
				В кредит от <span> <?php echo (int)(((int)$arResult['MIN_PRICE']['PRINT_DISCOUNT_VALUE'])/12);?> р/мес.</span>
			</div-->
			<div style="clear:both"></div>
...
?>
            ...
            <?if($arResult["akbDiscount"]):?>
			<div class="sell-old-akb">
            	<input type="checkbox" name="oldAkb" id="oldAkbChk" data-discount="<?=$arResult["akbDiscount"]?>" />
                <label for="oldAkbChk">Я сдаю старый аккумулятор (только для Таганрога и Ростова)</label>
            </div>
            <?endif;?>

            <div class="clb"></div>
            
            <?if(count($arResult['STORE_QUANTITY'])):?>
            <div class="storeQuantityList">
                <div class="storeQuantityTitle">Наличие в магазинах</div>
                <?foreach($arResult['STORE_QUANTITY'] as $sq):?>
                	<?
                    	$qtyClass = $sq["AMOUNT"]>8 ? "sqi-qty-high" : ($sq["AMOUNT"]>0 ? "sqi-qty-low" : "sqi-qty-none");
						$qtycaption = ($qtyClass=="sqi-qty-none") ? "нет" : "да";
					?>
                    <div class="storeQuantityItem">
						<div class="sqi-name"><?=$sq["STORE_NAME"]?></div>
                        <div class="sqi-instock <?=$qtyClass?>">
                        	<?=$qtycaption?> (<?=$sq["AMOUNT"]?>)
                        </div>
                    </div>
                <?endforeach;?>
            </div>
            ...
            <?
                $i=0;
                    ...
                    {if($arOneProp['CODE'] == 'CML2_BASE_UNIT' || $arOneProp['CODE'] == 'CML2_TRAITS' || $arOneProp['CODE'] == 'CML2_TAXES') continue;
                    if(in_array($arOneProp['CODE'],array("DLINA","SHIRINA","VYSOTA"))){
                        $gabarites[$arOneProp['CODE']] = $arOneProp['DISPLAY_VALUE'];
                        continue;
                    }
            ?>      <div class="property">
                    ...
                    */
                    $i++;
                if ($i == 5) echo "<div class='more-options'>"; 
                    }
                    if(count($gabarites)){?>
                        <div class="property">
                            ...
                            <div class="dotted"></div>
                            <div class="value">
                                <?=$gabarites["DLINA"]?>x<?=$gabarites["SHIRINA"]?>x<?=$gabarites["VYSOTA"]?> мм
                            </div>
                        </div>
                    <?}
                    echo "</div>";
                    echo "<div class='show-all-options'>Все характеристики</div>";
                    unset($arOneProp);
            ?>
...
<script type="text/javascript" src="http://delmex.ru/bitrix/templates/accumulators/components/bitrix/catalog.element/akb.default/jcarousellite_1.0.1.js"></script>
<script type="text/javascript">
...
$(".show-all-options").click(function(){
    $(".show-all-options").css('display','none');
    $(".more-options").toggle(500);
});
$(".show-review").click(function(event){
    event.preventDefault();
    $(".revuews-section-container").toggle(500);
    
    $("#bxlhe_frame_LHEBlogCom").append($(".lha-source-div").html());
    $(".lha-source-div").remove();
    $(".bxlhe-frame-table").css('display','none');
    //$("#bxlhe_frame_LHEBlogCom").append('<iframe id="LHE_iframe_LHEBlogCom" class="lha-iframe" ><html><body></body></html></iframe>');

    //$(".bxlhe-frame-table").remove();
    
});

//$('#post-button').click(function(){ alert();
$('body').on('click','.blog-comment-buttons',function(){ 
    $('#LHE_iframe_LHEBlogCom').contents().find('body').append($('#comment').val());
});
...
<?$APPLICATION->IncludeComponent(
	...
	array(
		...
		"IBLOCK_ID" => (CSite::InDir('/catalog/')) ? "1" : "6",
